...
export default function App() {
  ...
  if(status==="preview"&&preview){
    const p=preview;
    const tabs=[{i:"📊",l:t.pres},{i:"📄",l:t.doc},{i:"📝",l:t.cv},{i:"📋",l:"Submission Notes"}];
    return(
      <div style={S.page}><div style={{...S.container,maxWidth:680}}>
        ...
        <div style={S.pCard}>
          ...
          {tab===3&&<><div style={{textAlign:"center",marginBottom:14}}><span style={{fontSize:9,background:ac+"15",color:ac,display:"inline-block",padding:"3px 10px",borderRadius:6,fontWeight:700,marginBottom:6}}>📋 LMS Assignment Submission Fields</span><div style={{fontSize:16,fontWeight:800,color:"#eee"}}>Copy & Paste for Assignment Details</div></div><Sec t="Assignment Name (required)" ac={ac}><div style={{display:"flex",gap:8,alignItems:"center",background:"rgba(255,255,255,0.02)",border:`1px solid ${ac}22`,borderRadius:8,padding:10}}><input readOnly value={p.assignment_title || `${p.name} — Capstone Project Submission`} style={{flex:1,background:"transparent",border:"none",color:"#eee",fontSize:13,outline:"none"}}/><button onClick={()=>{navigator.clipboard.writeText(p.assignment_title || `${p.name} — Capstone Project Submission`);alert("Copied Assignment Name!")}} style={{fontSize:11,background:ac,color:"#fff",border:"none",padding:"5px 12px",borderRadius:6,cursor:"pointer",fontWeight:700}}>📋 Copy Title</button></div></Sec><Sec t="Description" ac={ac}><div style={{background:"rgba(255,255,255,0.02)",border:`1px solid ${ac}22`,borderRadius:8,padding:12}}><textarea readOnly value={p.assignment_description || `Student Name: ${p.name}\nEmail: ${p.email}\nInstitution: ${p.country}\n\nAttached Deliverables:\n1. Presentation Deck (.pptx)\n2. Professional Resume (.docx)\n3. Portfolio Archive (.zip)`} rows={10} style={{width:"100%",background:"transparent",border:"none",color:"#ccc",fontSize:12,fontFamily:"monospace",outline:"none",resize:"vertical"}}/><button onClick={()=>{navigator.clipboard.writeText(p.assignment_description || `Student Name: ${p.name}`);alert("Copied Description!")}} style={{marginTop:8,fontSize:11,background:ac,color:"#fff",border:"none",padding:"6px 14px",borderRadius:6,cursor:"pointer",fontWeight:700}}>📋 Copy Description</button></div></Sec></>}
        </div>
